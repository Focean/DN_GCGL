...
{
    public partial class FZB_处置形式 : Form
    {
        public FZB_处置形式()
        {
            InitializeComponent();
        ...
        {
            using (var form = new BZ_处置形式_Editor())
            {
                form.Editor_Add();
        ...
        private void Btn删除_Click(object sender, EventArgs e)
        {
            ...
            //
            string iRowID = ((DataTable)this.dgvList.DataSource).Rows[this.dgvList.CurrentRow.Index]["处置形式编码"].ToString();
            if (iRowID == null) return;
            //
            if (!AppServer.DialogMsg("确认要删除当前处置形式吗？", "删除确认")) return;
