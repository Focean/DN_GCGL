...
namespace GCGL_Client.Main
{
    public partial class Man_UserRole : Form
    {
        ...
        //修改状态
        public void Editor_Mod(string AUserCode, string AUserName, string ARoleList)
        {
            this.Text = string.Format("用户“{0}”的角色", AUserName);
            FUserCode = AUserCode;

            foreach (ListViewItem lvi in this.listView.Items)
            {
                if (("," + ARoleList + ",").IndexOf("," + lvi.SubItems[2].Text + ",") >= 0)
                {
                    lvi.Checked = true;
                }
                else
        ...
        private void btnDelRole_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                var dtm = new Ref_WS_GCGL.DataType_Role();
                dtm.ExAction = "Del";
                dtm.RoleID = iRoleID;
