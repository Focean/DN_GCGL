...
namespace BZFB_Service
{
    ...
    //{
    //    public static SqlDatabase db = DatabaseFactory.CreateDatabase("Connect_JXC") as SqlDatabase;
        ...
        private static object GetModelValue(object model, SqlDbType p_type, string p_name)
        {
            ...
            foreach (System.Reflection.PropertyInfo attr in properties)
            {
                if (attr.Name.ToLower() == p_name.ToLower())
                {
                    ...
                    if (p_type == SqlDbType.UniqueIdentifier && result != null)
                        result = SqlGuid.Parse(result.ToString());
                    break;
                }
            }
            return result;
